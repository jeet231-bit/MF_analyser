...
### 2026-09-11 — Phase 2 complete (commit 2049186)
Custom Excel formula grammar (tokenizer + Pratt parser, R1C1-relative ASTs) — `formulas` library NOT wrapped (its schedula graph translation cost more than a 20-function grammar). Two-stage template keying; maximal-rectangle formula blocks; networkx over blocks only, cell-level derived lazily; classification, labels, sheet roles with per-version overrides; rule extraction per template. 102 tests + opt-in scale test.

**Real-workbook metrics:** 10 in-scope sheets (sheetScope in dashboard.config.json; 39 total), 145 templates in scope (282 workbook-wide), 467,289/467,289 formula cells covered, 193 formula blocks, 50 input blocks, 226 business rules, 0 parse errors, interpretation 16–22s, peak 266 MB.

**Findings:** (1) Genuine circular reference in master — Bull-Bear Returns AZ3/AZ4 (COUNT of AJ) sit in rows 3–4 scanned by HLOOKUP($3:$4) feeding W:AA which derives AJ. Excel tolerates it (no iterative calc). **Decision: fix in the master** (move AZ3/AZ4 out of rows 3–4) rather than build iterative seeding into the engine; engine keeps report-and-refuse cycle guard. (2) Sheet roles classified: MF Master + Domain = calculation, Roll ×3 + First NAV = input, Composite Ranks = output, 4 performance sheets = calculation — to be verified/corrected by researcher in the Phase 6 overview. (3) samples/master.xlsx must be a copy, never the live OneDrive file Excel has open (file locks break uploads).

**Next:** Phase 6 pulled forward (shell + design system + overview with role-override UI, disabled future modules) — prompt issued. Then amended Phase 3: vectorised template-by-template evaluation, clean-DAG assumption once master is fixed.

### 2026-09-11 — Cycle fixed in master; Phase 6 shell delivered
- samples/master.xlsx: AZ3/AZ4 moved to AZ1/AZ2 at sheet-XML level (formula text byte-identical). Result: 145 templates, 193 blocks, ZERO cycles, 16s interpret. Same two-cell move to be applied in the real master; cycle report-and-refuse guard stays.
- Scope confirmed upstream-closed. Excluded-but-analytical sheets: CY Returns (156k formulas), Summary-Performance (98k), Report (42k), Nifty (26k) + 6 smaller; other 19 are pivots/static pulls. DECISION DEFERRED to before Phase 7: extend sheetScope with CY/Report layer if that's what the team consumes. Extending is additive.
- Phase 6 shell + overview live: tokens, components, stat tiles, SVG sheet flow by topo depth, editable roles table (override endpoint), rules grouped by sheet, cycle chip, upload flow. Heuristic improved generally (self-contained sheet read only via lookups = reference) — MF Master + Domain now reference; all 10 roles verified correct. All 226 rules read by researcher: no template misreads.
- **Master data defect found via review:** Roll Perf row 3181 ("WhiteOak Capital Aggressive Hybrid Fund - Reg - Growth") has K:U referencing row 3258 (Nifty Smallcap 250 TRI) — cut-paste damage; fund shows the index's rolling returns/quartile. Bull-Bear 3182 same signature but harmless mirror. TO FIX in real master; engine never compensates (fidelity contract).
- **Phase 4 addition (agreed):** validation report gains a structural-anomalies section — block-fragmentation outliers + label-vs-referenced-row mismatches — to auto-catch the WhiteOak class of defect on every upload.
- Amended Phase 3 prompt issued: block-topological, template-at-a-time vectorised execution (numpy; precomputed lookup indexes), clean-DAG assumption, Excel semantics, full+incremental runs, budgets (full ≤90s profiled, incremental ≤5s), lineage endpoint, sampled 5k-cell real-workbook reconciliation smoke test.

### 2026-09-11 — Master workbook repaired in Cowork session
User uploaded the master (AZ move already applied by user). XML-level repairs made and returned as "Equity MF Analyser-2026-08-31-FIXED.xlsx":
- Roll Perf row 3181: 9 cells (K,L,M,N,O,P,Q,S,U) had relative refs to row 3258 (Nifty Smallcap 50 row) — retargeted to 3181; absolute range ends ($...$3258) correctly left untouched; R,T were intact shared members.
- Bull-Bear Returns row 3182: NOT harmless after all — same cut-paste damage, 37 cells referencing row 3259 retargeted to 3182. Verified against shared masters (P{r}=A{r} own-row pattern, si1790 anchored P3152).
...
### 2026-09-11 — Phase 4 complete (commit 3b7c3d2)
Validation formalised: 467,289/467,289 reconciled on the FIXED master (15-sig-digit numeric policy), 0 skipped, 0 cycles/fragments/own-row violations, 24s, status passed-with-warnings (5 duplicate-key groups), version activated. Regression works: old version flags exactly Roll Perf 3181 + Bull-Bear 3182. Four anomaly detectors live (pattern breaks, own-row, duplicate keys, stale cache) + activation gate + Validation UI module with sidebar badge.
**Researcher action:** Domain rows 386–397 — six index funds duplicated as lookup keys (Axis Nifty Energy, Edelweiss Nifty REITs, Kotak Nifty Bank, Navi Nifty REITs, UTI BSE India Sector Leaders, UTI Nifty 500); first row silently wins — dedupe in master if classifications differ. Header repeats harmless (Phase 9 polish for the filter).
Phase 5 prompt issued: three-group diff (DATA extent vs LOGIC templates/rules vs STRUCTURAL sheets/blocks/anomaly-deltas), impact via block DAG, pending-review upload flow, Versions UI, real regression pair (defective vs fixed version → exactly the 2 repaired rows, anomaly delta resolved).

### 2026-09-11 — Phase 5 complete (commit b1b6a19)
Three-group diff live (DATA/LOGIC/STRUCTURAL). Real regression diff (defective vs fixed master): 2 logic changes (one per repaired row, grouped per-row with per-column payload), 46 cells, 13 of 34 outputs affected, 6 structural notes incl. 4 anomalies resolved. Upload flow now auto interpret→validate→diff→pending-review; activation/rollback in Versions module. 142 backend + 22 frontend tests.
**SCOPE DECISION (user):** Add Report + CY branch. Phase 7 issued as two stages: Stage A = extend sheetScope (Report, CY Returns, Summary-Performance, Nifty + upstream closure), implement any new registry functions, 100% validation required before UI; Stage B = Inputs/Calculations/Outputs modules per build kit, Outputs leads with Report layer, lineage panel, what-if incremental runs synchronous with quiet progress (background job + polling if any full run from UI exceeds ~10s).

### 2026-09-11 — Phase 7 Stage A: Averages cycle fixed in Cowork session
User's uploaded master still had the cycle (the row 3181/3182 repairs were a separate, earlier fix). Applied at XML level, delivered as "Equity MF Analyser-master-CYCLEFIX.xlsx":
- Averages!F8:L49 — 280 AVERAGEIF formulas rebounded from whole Report columns to the fund rows: `AVERAGEIF(Report!$AC$10:$AC$1509,$C8,Report!G$10:G$1509)`. Column stays relative, rows absolute. Zero whole-column Report refs remain in Averages.
- Values unchanged by construction (criterion "Regular-Aggressive Hybrid Fund" can never exact-match "Average-…" labels in AC1510:AC1549); cached values preserved, fullCalcOnLoad="1" set.
- Prior repairs verified intact (Roll Perf K3181, Bull-Bear P3182, AZ1/AZ2).
- **New minor anomaly spotted:** Averages!G28 holds a stray `CONCATENATE("Average-",D30,"-",E30)` inside the F:L numeric block — a misplaced member of the B30:B49 pattern. Harmless but should be cleared; good test case for the fragmentation detector.
- **Role note for Stage A3:** Nifty is an isolated leaf (nothing in the closure reads it) so DAG-sink logic marks it an output, but it is raw index source data — consider overriding to reference in the overview, else Outputs shows a 13,209-row daily series section.
Phase 7 plan (Stage A scope closure + A2 checkpoint + A3 re-interpret/validate/activate; Stage B Inputs/Calculations/Outputs, grid window API, lineage trace narration, background runs) reviewed and approved as written. Scope closure = 15 sheets (adds Averages), 789,828 formulas, 223 templates, all functions inside the existing 20-function contract.

### 2026-09-11 — Phase 7 Stage A complete (commit bf527d0), activation blocked on stale cache
Scope = 15 sheets (closure adds Averages only), 789,828/793,919 formula cells, 223 templates / 250 blocks / 69 input blocks. Interpret 35s/333MB; full run 7–9s/310MB; incremental 1.4s typical, 4.5s worst (MF Master text key, 128 blocks). Roles as agreed; Nifty configured as reference (isolated leaf). No registry additions — new sheets stay inside the 20-function contract.
**Two engine fixes:** (1) Excel sums one double at a time; numpy pairwise summation differed in the 15th digit on 20 AVERAGEIF cells — SUM/SUMIF/AVERAGEIF/SUMPRODUCT now sum in Excel's order, fixture-pinned. (2) A cyclic version previously couldn't be validated at all; now validates as FAILED with anomaly report intact and can never be activated, override or not.
**Blocker — 28 stale cached values (verified independently in Cowork):** Averages F22:L22 and F44:L44 cache "--" for Regular-/Direct-Multi Asset Allocation; 35 Report rows carry each key with 4 numeric G values, so Excel's AVERAGEIF gives 18.0075 (Regular) and 18.8725 (Direct). Engine correct, cache stale — cause is the old circular reference (Excel abandons the calc pass with a cycle and iterative calc off), so these have been frozen since before "Multi Asset Allocation" data arrived. Plus the 14 Report cells that look them up = 28 mismatches.
**THIRD REAL DEFECT for the team:** Report's two Multi Asset Allocation average rows have been showing "--" instead of numbers in the team's published output.
Resolution: open CYCLEFIX workbook in Excel → Ctrl+Alt+F9 → verify F22/F44 ≈ 18.01/18.87 → save → samples/master.xlsx. Explicitly NOT patching cached values in-session (validation must grade the engine against Excel, not against itself) and not recalculating via LibreOffice (different engine would become the baseline).
G28 stray: absent from the current file, nothing to detect. Old-vs-fixed diff now reads 3 logic changes (46 cells + 280-cell Averages range fix), both own-row anomalies resolved.

### 2026-09-11 — Deployment & storage decision
**Use case confirmed:** internal research engine, ~2 users. Master Excel defines the computational logic; anyone uploads a new Excel and the data points refresh for downstream use. Common case is a DATA refresh (same logic, new fund data), which the Phase 5 three-group diff already handles correctly; activation gate keeps uploads from going live unreviewed.
**Decision: no Supabase, no Postgres migration.** SQLite (with WAL) is correct for 2 users; a hosted DB would only add latency to reads that are currently ~14ms. What's actually needed is a HOST — one internal always-on machine running the backend, both users via browser over the LAN. That is a Phase 10 packaging job, not an architecture change. Revisit Postgres + object storage only if user count grows or data must be centralised. Note: Supabase could never run the engine anyway (300MB RSS, 7–9s full runs need a real compute host).
**Roadmap:** user chose to finish Phases 8 and 9 first; deployment/auth/scheduled-ingestion parked.
**Phase 9 additions flagged (2-user reality):** concurrent runs — two simultaneous what-ifs = two engine states at ~300MB each; review state_cache sizing and run queueing. Backend currently assumes a single process.
**Phase 8 prompt issued** with amendments: output sheets as default xlsx scope (full 790k-cell export opt-in), background job for exports over ~10s reusing the Phase 7B pattern, CSV = current grid window, PDF is a report (top-N / category summaries) never a 1,500-row dump, xlsx round-trip test.

### 2026-09-11 — Phase 7 complete (Stage B commit bbbe532); UI direction gap identified
Stage B delivered: Inputs module (labelled what-if levers — weights, as-of dates; header rows collapsed; master tables via editable grid), synchronous what-if runs (Roll Perf weight ~1s, master-table edit 6.5s, UI full recalc 15s as background job), Outputs (Report → Composite Ranks → Summary-Performance, stat tiles with signed deltas, changed-cell markers, "changed rows only"), lineage panel (re-evaluates the single cell with the real engine + trace hook — no second interpreter), Versions from Phase 5. 156 backend / 37 frontend tests clean. Baseline grids now cached after first what-if post-restart (~9s once, then clones). Incremental budget honestly re-split: <5s typical, <8s master-table key-column edit.
**USER FEEDBACK — important:** user judged the dashboard "not useful" from a Validation-module screenshot on a stale pre-Stage-B build (sidebar still showed "arrives with the engine"; 4,67,289 cells = old 10-sheet scope). Clarified. BUT the underlying critique is valid and is a gap in the original spec: even after Stage B the dashboard is SHEET-SHAPED (grids of Report/Composite Ranks), not RESEARCH-SHAPED. A fund analyst wants: fund explorer with search/filter by category/AMC/direct-regular, fund detail across all periods, category quartile distribution, top/bottom leaderboard, fund comparison, and rank movement vs previous version (impossible in Excel — uses the version history).
**Architectural resolution proposed:** a semantic mapping layer in dashboard.config.json (fund-name column, category, AMC, return/rank/quartile column roles) — config not code, so workbook-agnosticism holds while rich views become possible.
**Sequencing (user's call):** finish Phase 8 and Phase 9 first, THEN the research-views/UI phase. User has reference UI pictures to share at that point.
**Phase 8 amendment added:** export layer takes a VIEW DESCRIPTOR (rows, columns, labels, title, provenance) produced by the sheet grid, not sheets directly — so future non-sheet research views export through the same path.
**Fourth data finding for the team:** Alphagrep Multi Asset Allocation fund is absent from CY Returns, hence the two "--" category averages in both Excel and the app. Data-completeness question for the master.
**Also pending:** labelOverrides entries needed in dashboard.config.json for odd auto-derived labels (e.g. CY Returns as-of date reads "Scheme Name" from the text above it).

### 2026-09-11 — Phase 8 complete (commit 3a93c83)
All exports flow through one ExportView descriptor (title, labelled columns, row labels, rows, types, number formats, provenance) — produced by sheet grids AND the version changelog, so writers never touch sheets and future research views plug into the same path. Measured: xlsx output-sheets 1.2MB/6.6s sync; xlsx all-15-sheets 8MB/38s background job; CSV grid window 28KB/0.2s; changelog CSV+xlsx. Report PDF is a genuine report (headline tiles with deltas, category summary grouped by the most granular classification-like text column, top-20 by first numeric output column, largest moves vs baseline, rules by kind, changelog when relevant) — no paginated fund lists. xlsx round-trip passes at 15 significant digits (openpyxl writes 16). 164 backend / 41 frontend tests.
**Open item:** WeasyPrint can't load GTK's libgobject on this Windows machine → PDF returns 501 with instructions; HTML report renders fine in 2.2s. Fix = install GTK3 runtime for Windows (tschoonj installer), no code change. Flagged as a portability question for the deployment machine; documented fallback = headless Chromium print-to-PDF (HTML report already exists, contained change to one renderer function).
**Phase 9 prompt issued** with amendments: (1) SCOPE DISCIPLINE — do not polish the sheet-shaped views, the research-views layer supersedes them; correctness/robustness/concurrency/perf/deployment only. (2) Concurrency for 2 users — two what-ifs = two ~300MB engine states; state_cache sizing + run limit/queue with UI state; document or fix the single-process assumption; concurrent-client tests. (3) PDF portability confirmed + README prerequisite + Chromium fallback noted. (4) labelOverrides for every wrong auto-derived label. (5) Robustness incl. unrelated-workbook upload never half-activating. (6) QA.md measured timings for machine sizing. (7) Full e2e test. (8) docs/MASTER_FINDINGS.md — plain-language write-up of the four defects found in the master (Roll Perf 3181, Bull-Bear 3182, Averages circular ref, Alphagrep missing from CY Returns) for the research team.

### 2026-09-11/12 — Phase 10 complete (10A a8152ec/b3ba08b, 10B ffb90f5)
Research console built. Semantic map in dashboard.config.json (entity/dimensions/34 measures/phases/periods/categoryStats/insights) — all 34 measures resolved against the active version, zero problems. Declarative insight engine (where/groupBy/acrossVersions/aggregate + sentence templates, minGroupCount default 10). Per-version snapshots persisted at activation; summary warm 0.02s and flat in upload count. UI: dark rail with Research/Workbook switch, Dashboard, Insights, Funds (+ pivot row), Fund detail, Categories, Movement, Admin, Upload; 62 vitest + research suite green.
**Sentence review (2nd pass) applied:** long-term switched to rank_lt with category-leader framing; held-Q1 card suppressed until a second distinct as-of date; risk-adjusted volatility leg = top half; bear score described as a composite not a return; manager card labelled "management teams" with optional split (off); corpus formatted as ₹ lakh crore; zero variants + as-of dates in all narratives; minGroupCount on all superlatives.
**KEY FINDING — rating coverage:** only 1,434 of 3,232 funds (44%) carry a composite rating. Breakdown: 701 in categories with <4 ranked funds, **1,030 showing "--" for part of the composite**, 67 outside the universe flag. Unranked categories corrected 72 → 126 of 200 (engine definition switched to match the formula: ranked funds, not members).
**Two recommendations issued:**
 (a) Cross-reference the 1,030 unrated against the First NAV sheet (already in scope) to split "too young to rate" from "data missing that should be there" — the second list is a fifth MASTER_FINDINGS entry if non-empty. Requested as a coverage insight.
 (b) Methodology question for the research team: category key = Plan × Sub-Nature doubles and splinters every bucket, leaving 126/200 categories unranked. Consider ranking at sub-nature level pooled across Direct/Regular with plan as a dimension. Platform can now show both versions in minutes.
 Plus sectionLabels for insight section headings and the team-name grammar fix.
**Remaining work is operational:** first genuine monthly upload (activates Movement/consistency/diff for real — re-read the sentences then), and deployment (one always-on internal machine, SQLite+WAL, 2 users over LAN).