...
| 12 | Pivots: a cross-tab builder over the joined fund table with the change since last month in every cell, the master's 17 pivot layouts parsed from the OOXML pivot parts and recomputed behind it, and a dashboard that leads with what changed and where to look | done |
...
- **Dashboard**: what is true, then what changed, then where to look. The greeting ("Good morning, Jeet", from the browser clock and the viewer's name), the scope bar, the **executive summary** band (computed sentences leading with what is true: rated count, top quartile, who leads, the all-weather set, best value, the movement fact folded in), four KPI tiles with the coloured top rail (from `research.kpis` in the config), the quartile distribution with median and largest category, category leaders, the watchlist, and one quiet **coverage strip** at the bottom (a small bar, one sentence, and a link to the full breakdown in Admin). Counts that are only counts sit in a compact strip inside the summary band; below it a **what changed** band names what moved since the previous upload with its largest movers, and a row of **where to look** cards turns the insights named in `research.dashboard.callouts` into a count, a sentence and the list behind it. A card renders only when it has content; a zero-state fact is absorbed into the narrative.
...
- **Pivots**: one question in words, *show the [average] [composite rank] by [AMC], compared with [last month]*. Every fund in scope is grouped by anything the master knows (category, sub-nature, AMC, plan, universe flag, or a corpus or expense band), and each group shows its funds, its quartile mix, its top-quartile share, the measure you chose and its median position (rank as a share of the ranked funds in the fund's own category, so categories of different sizes compare fairly), **each with the change since the previous upload**. Six columns, no sideways scrolling, the leader shaded, and a legend above the table saying what each colour means. The workbook sets quartiles within each category, so when you group by something made of whole categories (category, plan, sub-nature) every group splits into four equal quarters by construction; the screen then hides the quartile columns and says why rather than showing 25 % as if it were a finding. Groups with fewer than the minimum rated funds are listed last and never shaded or ranked. Every insight and dashboard card also shows how it is counted, in the workbook's own sheet and column, so a name such as "All weather" can be checked in Excel. A sentence above the table names who leads and who moved most. Presets in `dashboard.config.json` are starting points, not screens. A row opens its own funds. The workbook's seventeen frozen pivot layouts sit behind a fold at the bottom, recomputed from today's numbers with the date Excel last refreshed its own copy; each says whether it reads engine values or cached ones. Exports as xlsx or csv with every group and its change.
