...
- **Pivots**: one question in words, *show the [average] [composite rank] by [AMC], compared with [last month]*. Every fund in scope is grouped by anything the master knows (category, sub-nature, AMC, plan, universe flag, or a corpus or expense band), and each group shows its funds, its quartile mix, its top-quartile share, the measure you chose and its median rank, **each with the change since the previous upload**. Six columns, no sideways scrolling, the leader shaded. A sentence above the table names who leads and who moved most. Presets in `dashboard.config.json` are starting points, not screens. A row opens its own funds. The workbook's seventeen frozen pivot layouts sit behind a fold at the bottom, recomputed from today's numbers with the date Excel last refreshed its own copy; each says whether it reads engine values or cached ones. Exports as xlsx or csv with every group and its change.
