...
Internal "Excel-driven research analytics platform". Monorepo: `backend/` (Python 3.12, FastAPI, Pydantic v2, uv) and `frontend/` (React 18, TypeScript, Vite, Tailwind, Recharts). The build proceeds in numbered phases 0–9, specified with acceptance criteria in `docs/BUILD_KIT.md`; one phase per session, one commit per phase. Phases 0, 1, 2, 6 (shell + design system + overview only), 3, 4 and 5 are done; Phase 7 runs in two stages: Stage A (scope extension to the Report layer, done) and Stage B (Inputs, Calculations, Outputs modules). Agreed order from here: **7B → 8 → 9**.
