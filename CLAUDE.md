...
- A model with circular references still validates (Phase 7A): the structural anomaly report is produced, reconciliation is skipped, the report is `failed` with `run_id = None` and the cycle descriptions as reasons, and such a version can never be activated, override or not. Stale cached values are a master defect too: after a file-level formula edit, Excel must do a full recalculation (Ctrl+Alt+F9) before the copy is dropped, or the validator compares against pre-edit numbers. Phase 3 must evaluate template-by-template over ranges (vectorised where possible), never cell by cell, and may assume a clean DAG: the master's one circular reference was fixed in the file (`samples/master.xlsx`); cycles are still reported readably and refused.
